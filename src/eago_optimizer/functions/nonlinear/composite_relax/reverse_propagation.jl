...
function r_init!(t::Relax, g::DAT, c::RelaxCache{V,N,T}) where {V,N,T<:RelaxTag}
    if !_is_num(c, 1)
...
function rprop!(t::Relax, v::Variable, g::DAT, c::RelaxCache{V,N,T}, k::Int) where {V,N,T<:RelaxTag}
    i = _first_index(g, k)
    x = _val(c, i)
...
function rprop!(t::Relax, v::Subexpression, g::DAT, c::RelaxCache{V,N,T}, k::Int) where {V,N,T<:RelaxTag}
...
"""
function rprop_2!(t::Relax, v::Val{PLUS}, g::DAT, c::RelaxCache{V,N,T}, k::Int) where {V,N,T<:RelaxTag}
...
for (f, fc, F) in ((-, MINUS, IntervalContractors.minus_rev),
    ...
    eval(quote 
        function rprop!(t::Relax, v::Val{$fc}, g::DAT, b::RelaxCache{V,N,T}, k::Int) where {V,N,T<:RelaxTag}
            _is_num(b,k) && (return true)
            x = _child(g, 1, k)
            y = _child(g, 2, k)
