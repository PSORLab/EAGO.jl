...
function fprop!(::Type{Relax}, ::Type{Subexpression}, g::DAG, c::RelaxCache{V}, k::Int) where V
...
for (f, F) in ((fprop_2!, +), (fprop_2!, *), (fprop_2!, min), (fprop_2!, max),
               (fprop!, -), (fprop!, /))
    @eval function ($f)(::Type{Relax}, ::typeof($F), g::DAG, b::RelaxCache{V,S}, k::Int) where {V,S}
        x = _child(g, 1, k)
        y = _child(g, 2, k)
...
for (F, SV, NV) in ((+, :(zero(V)), :(zero(S))),
                    ...
                    (min, :(inf(V)), typemax(S)),
                    (max, :(-inf(V)), typemin(S)))
    @eval function fprop_n!(::Type{Relax}, ::typeof($F), g::DAG, b::RelaxCache{V,S}, k::Int) where {V,S}
        z = $SV
        znum = $NV
...
for F in (+, *, min, max)
    @eval function fprop!(::Type{Relax}, ::typeof($F), g::DAG, b::RelaxCache{V,S}, k::Int) where {V,S}
        n = _arity(g, k)
        if n == 2
...
function fprop!(::Type{Relax}, ::typeof(^), g::DAG, b::RelaxCache{V,S}, k::Int) where {V,S}
    x = _child(g, 1, k)
    y = _child(g, 2, k)
    x_is_num = is_num(b, x)
    y_is_num = is_num(b, y)
...
function fprop!(::Type{Relax}, ::typeof(user), g::DAG, b::RelaxCache{V,S}, k::Int) where {V,S}
...
function fprop!(::Type{Relax}, ::typeof(usern), g::DAG, b::RelaxCache{V,S}, k::Int) where {V,S}
