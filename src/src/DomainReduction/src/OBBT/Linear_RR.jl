...
function STD_Linear_RR!(X::Vector{V},opt,UBD::Float64) where {V<:AbstractInterval}
    ...
    x_mc::Vector{HybridMC{opt[1].numVar,V,Float64}} = [HybridMC{opt[1].numVar,V,Float64}(SMCg{opt[1].numVar,V,Float64}(x0[i],x0[i],seed_g(Float64,i,opt[1].numVar),seed_g(Float64,i,opt[1].numVar),X[i],false)) for i=1:opt[1].numVar]
    #f_mc::SMCg{opt[1].numVar,V,Float64} = opt[1].f(x_mc)
    f_mc::HybridMC{opt[1].numVar,V,Float64} = opt[1].f(x_mc)
    if opt[1].numConstr>0
        #c::Vector{SMCg{opt[1].numVar,V,Float64}} = opt[1].g(x_mc)
        c::Vector{HybridMC{opt[1].numVar,V,Float64}} = opt[1].g(x_mc)
...
function Imp_Linear_RR!(X::Vector{V},opt, UBD::Float64) where {V<:AbstractInterval}
    ...
    p_mc::Vector{HybridMC{opt[1].numVar,V,Float64}} = [HybridMC{opt[1].numVar,V,Float64}(SMCg{np,V,Float64}(x0[i],x0[i],seed_g(Float64,opt[1].numVar,i),seed_g(Float64,opt[1].numVar,i),X[i],false)) for i=1:opt[1].numVar]
    ...
    f_mc::HybridMC{np,V,Float64} = opt[1].solver.ImplicitOpts.f(x_mc[end],p_SMC)
    c::Vector{HybridMC{np,V,Float64}} = opt[1].solver.ImplicitOpts.g(x_mc[end],p_SMC)

    # UPDATE gL_loc & gU_loc
    gL_temp = filter(x->(x>nx),opt[1].gL_loc)
    gU_temp = filter(x->(x>nx),opt[1].gU_loc)

    c_cv::Vector{Float64} = [c[i].SMC.cv for i=1:opt[1].numConstr]
    c_cc::Vector{Float64} = [c[i].SMC.cc for i=1:opt[1].numConstr]
    dcdx_cv::Array{Float64,2} = Float64[c[i].SMC.cv_grad[j] for i=1:opt[1].numConstr,j=1:np]
    dcdx_cc::Array{Float64,2} = Float64[-c[i].SMC.cc_grad[j] for i=1:opt[1].numConstr,j=1:np]
    cval = vcat(c_cv[gU_temp-nx]-opt[1].gU[gU_temp-nx],-c_cc[gL_temp-nx]+opt[1].gL[gL_temp-nx])
    dcdx::Array{Float64,2} = vcat([f_mc.SMC.cv_grad[i] for i=1:np]',
    ...
    rhs = vcat([UBD + f_mc.SMC.cv - sum([x0[i]*f_mc.SMC.cv_grad[i] for i=1:opt[1].numVar])],
