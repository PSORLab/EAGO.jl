...
function impRelax_f(f::Function,h::Function,hj::Function,X::Vector{V},
                    ...
                    mc_opt::mc_opts{T},param::Vector{Vector{HybridMC{N,V,T}}}) where {N,V,T<:AbstractFloat}
  np::Int64 = length(P)
  sone::SVector{np,T} = ones(SVector{np,T})
  p_mc::Vector{HybridMC{np,V,T}} = [HybridMC{np,V,T}(SMCg{np,V,T}(p[i],p[i],sone,sone,V(P[i].lo,P[i].hi),false)) for i=1:np]
  xpMC::Vector{HybridMC{np,V,T}} = MC_impRelax(h,hj,p_mc,pmid,X,P,mc_opt,param)
  return f(xpMC,p_mc)
end
