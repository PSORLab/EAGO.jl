...
function +(x::SMCg{N,V,T},y::C) where {N,V,T<:AbstractFloat,C<:AbstractFloat}
	f::T = convert(T,y)
	return SMCg{N,V,T}(x.cc+f, x.cv+f, x.cc_grad, x.cv_grad, (x.Intv+f),
					   x.cnst, x.IntvBox,x.xref)
end
...
function -(x::SMCg{N,V,T},c::T) where {N,V,T<:AbstractFloat}
	return SMCg{N,V,T}(x.cc-c, x.cv-c, x.cc_grad, x.cv_grad, (x.Intv-c),
	            x.cnst, x.IntvBox,x.xref)
end
...
function *(x::SMCg{N,V,T},c::T) where {N,V,T<:AbstractFloat}
	if (c>=zero(T))
		return SMCg{N,V,T}(c*x.cc,c*x.cv,c*x.cc_grad,c*x.cv_grad,c*x.Intv,x.cnst,x.IntvBox,x.xref)
	else
		return SMCg{N,V,T}(c*x.cv,c*x.cc,c*x.cv_grad,c*x.cc_grad,c*x.Intv,x.cnst,x.IntvBox,x.xref)
	end
end
function *(c::T,x::SMCg{N,V,T}) where {N,V,T<:AbstractFloat}
	if (c>=zero(T))
...
function *(x::SMCg{N,V,T},c::C) where {N,V,T<:AbstractFloat,C<:AbstractFloat}
	if (c>=zero(C))
		return SMCg{N,V,T}(convert(T,c*x.cc),convert(T,c*x.cv),c*x.cc_grad,c*x.cv_grad,c*x.Intv,x.cnst,x.IntvBox,x.xref)
	else
		return SMCg{N,V,T}(convert(T,c*x.cv),convert(T,c*x.cc),c*x.cv_grad,c*x.cc_grad,c*x.Intv,x.cnst,x.IntvBox,x.xref)
	end
end
function *(c::C,x::SMCg{N,V,T}) where {N,V,T<:AbstractFloat,C<:AbstractFloat}
	if (c>=zero(C))
