...
function LP_Relax_LBD(X::Vector{Interval{Float64}},
        ...
        x_mc::Vector{HybridMC{opt[1].numVar,Interval{Float64},Float64}} = [HybridMC{opt[1].numVar,Interval{Float64},Float64}(
                                                                           SMCg{opt[1].numVar,Interval{Float64},Float64}(x0[i],
                                                                                                                         x0[i],
                                                                                                                         seed_g(Float64,i,opt[1].numVar),
                                                                                                                         seed_g(Float64,i,opt[1].numVar),
                                                                                                                         X[i],
        ...
        #f_cv::Float64 = f.cv
        f::HybridMC{opt[1].numVar,Interval{Float64},Float64} = opt[1].f(x_mc)
        f_cv::Float64 = f.SMC.cv

        if opt[1].numConstr>0
            #c::Vector{SMCg{opt[1].numVar,Interval{Float64},Float64}} = opt[1].g(x_mc)
            c::Vector{HybridMC{opt[1].numVar,Interval{Float64},Float64}} = opt[1].g(x_mc)
        ...
        if opt[1].numConstr>0
            cx_ind2::Int64 = 1
            for i in opt[1].gU_loc
                rhs[cx_ind2] = sum(x0[:].*c[i].SMC.cv_grad[:])+min(c[i].SMC.Intv.hi,opt[1].gU[i])-c[i].SMC.cv
                cx_ind2 += 1
            end
            for i in opt[1].gL_loc
                rhs[cx_ind2] = sum(-x0[:].*c[i].SMC.cc_grad[:])-max(c[i].SMC.Intv.lo,opt[1].gL[i])+c[i].SMC.cc
                cx_ind2 += 1
            end
        end

        model = buildlp([f.SMC.cv_grad[i] for i=1:opt[1].numVar], dcdx, '<', rhs, l, u, opt[1].solver.LP_solver)
        result = solvelp(model)

        # Unpacks the results from the LP solver
        if (result.status == :Optimal)
            val::Float64 = max(f.SMC.Intv.lo,result.objval + f_cv - sum([x0[i]*f.SMC.cv_grad[i] for i=1:opt[1].numVar]))
...
function LP_Relax_LBD(X::Vector{MCInterval{Float64}},
        ...
        x_mc::Vector{SMCg{opt[1].numVar,MCInterval{Float64},Float64}} = [SMCg{opt[1].numVar,MCInterval{Float64},Float64}(x0[i],
                                                                            ...
                                                                            false) for i=1:opt[1].numVar]
        =#
        x_mc::Vector{HybridMC{opt[1].numVar,MCInterval{Float64},Float64}} = [HybridMC{opt[1].numVar,MCInterval{Float64},Float64}(
                                                                           SMCg{opt[1].numVar,MCInterval{Float64},Float64}(x0[i],
                                                                                                                           x0[i],
                                                                                                                           seed_g(Float64,i,opt[1].numVar),
                                                                                                                           seed_g(Float64,i,opt[1].numVar),
                                                                                                                           X[i],
        ...
        f::HybridMC{opt[1].numVar,MCInterval{Float64},Float64} = opt[1].f(x_mc)
        f_cv::Float64 = f.SMC.cv

        if opt[1].numConstr>0
            #c::Vector{SMCg{opt[1].numVar,MCInterval{Float64},Float64}} = opt[1].g(x_mc)
            c::Vector{HybridMC{opt[1].numVar,MCInterval{Float64},Float64}} = opt[1].g(x_mc)
            dcdx::SparseMatrixCSC{Float64,Int64} = spzeros(length(opt[1].gL_loc)+length(opt[1].gU_loc),opt[1].numVar)
        else
            dcdx = spzeros(1,opt[1].numVar)
        end
        ...
        if (result.status == :Optimal)
            ...
            mult::Vector{Float64} = result.attrs[:redcost]
                        if (opt[1].numConstr > 0)
                          GInt::Vector{MCInterval{Float64}} = opt[1].g(X)
                          cInt::Vector{MCInterval{Float64}} = vcat(GInt[opt[1].gU_loc]-opt[1].gU[opt[1].gU_loc],
                                                -GInt[opt[1].gL_loc]+opt[1].gL[opt[1].gL_loc])
                          for i=1:opt[1].gexp
                            if (cInt[i].lo>0.0)
                                val = -Inf
