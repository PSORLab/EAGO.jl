...
function snopt_callback_LBD_Imp(y::Vector{Float64},
    ...
    nx::Int64 = opt.solver.Implicit_Options.nx
    ...
    if opt.solver.Implicit_Options.numConstr>0
        if (opt.solver.Implicit_Options.Inplace)
            f_val::SMCg{np,Interval{Float64},Float64},c::Vector{SMCg{np,Interval{Float64},Float64}} = NdimpRelax_fg(opt.solver.Implicit_Options.f,
                                                                               opt.solver.Implicit_Options.g,
                                                                               opt.solver.Implicit_Options.h,
                                                                               opt.solver.Implicit_Options.hj,
                                                                               Y[1:nx],
                                                                               Y[(nx+1):opt.numVar],
                                                                               y,
                                                                               pmid,
                                                                               opt.solver.Implicit_Options.opts,
                                                                               param)
        else
            f_val,c = impRelax_fg(opt.solver.Implicit_Options.f,
                                  opt.solver.Implicit_Options.g,
                                  opt.solver.Implicit_Options.h,
                                  opt.solver.Implicit_Options.hj,
                                  Y[1:nx],
        ...
        if (opt.solver.Implicit_Options.Inplace)
            f_val = NdimpRelax_f(opt.solver.Implicit_Options.f,
                                opt.solver.Implicit_Options.h,
                                opt.solver.Implicit_Options.hj,
                                Y[1:nx],
                                Y[(nx+1):opt.numVar],
            ...
            f_val = impRelax_f(opt.solver.Implicit_Options.f,
                               opt.solver.Implicit_Options.h,
                               opt.solver.Implicit_Options.hj,
                               Y[1:nx],
                               Y[(nx+1):opt.numVar],
    ...
    if opt.solver.Implicit_Options.numConstr>0
        cx_ind1::Int64 = 1
        for i in opt.solver.Implicit_Options.gL_loc
            for j=1:np
        ...
        for i in opt.solver.Implicit_Options.gU_loc
            for j=1:np
    ...
    if opt.solver.Implicit_Options.numConstr>0
        cx_ind2::Int64 = 1
        for i in opt.solver.Implicit_Options.gU_loc
            rhs[cx_ind2] = sum(pmid[:].*c[i].cv_grad[:])+opt.solver.Implicit_Options.gU[i]-c[i].cv
            cx_ind2 += 1
        end
        for i in opt.solver.Implicit_Options.gL_loc
            rhs[cx_ind2] = sum(-pmid[:].*c[i].cc_grad[:])-opt.solver.Implicit_Options.gL[i]+c[i].cc
            cx_ind2 += 1
        end
    end
...
function snopt_callback_LBD_Imp(y::Vector{Float64},
    ...
    if opt.solver.Implicit_Options.numConstr>0
        if (opt.solver.Implicit_Options.Inplace)
            f_val::SMCg{np,MCInterval{Float64},Float64},c::Vector{SMCg{np,MCInterval{Float64},Float64}} = NdimpRelax_fg(opt.solver.Implicit_Options.f,
                                                                               opt.solver.Implicit_Options.g,
                                                                               opt.solver.Implicit_Options.h,
                                                                               opt.solver.Implicit_Options.hj,
                                                                               Y[1:nx],
