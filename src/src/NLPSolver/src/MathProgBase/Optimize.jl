...
function MathProgBase.optimize!(s::EAGO_NLP_Model)
    ...
    if (s.Opts.solver.LBD_func_relax == "NS-STD")
        EAGO.set_diff_relax(0)
        EAGO.set_multivar_refine(false,1E-15)
    elseif (s.Opts.solver.LBD_func_relax == "NS-MV")
        EAGO.set_diff_relax(0)
        EAGO.set_multivar_refine(true,1E-15)
    elseif (s.Opts.solver.LBD_func_relax == "Diff1-MV")
        EAGO.set_diff_relax(1)
        EAGO.set_multivar_refine(true,1E-15)
    elseif (s.Opts.solver.LBD_func_relax == "Diff2-MV")
