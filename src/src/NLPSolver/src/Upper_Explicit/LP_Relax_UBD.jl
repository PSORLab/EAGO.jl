...
"""
function LP_Relax_UBD(X::Vector{Interval{Float64}},k::Int64,pos::Int64,opt::Any,temp::Any)

    #println("START UPPER BOUNDING PROBLEM!")
    l::Vector{Float64} = [X[i].lo for i=1:opt[1].numVar]
    u::Vector{Float64} = [X[i].hi for i=1:opt[1].numVar]
    x0::Vector{Float64} = (l + u)/2.0
    x_mc::Vector{SMCg{opt[1].numVar,Float64}} = [SMCg{opt[1].numVar,Float64}(x0[i],
                                                                        x0[i],
    ...
    f::SMCg{opt[1].numVar,Float64} = opt[1].f(x_mc)
    f_cc::Float64 = f.cc
    ...
    model = buildlp([-f.cc_grad[i] for i=1:opt[1].numVar], dcdx, '<', rhs, l, u, opt[1].solver.LP_solver)
    result = solvelp(model)

    if (result.status == :Optimal)
        val::Float64 = f_cc - result.objval - sum([x0[i]*f.cc_grad[i] for i=1:opt[1].numVar])
